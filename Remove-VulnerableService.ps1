...
<#
.SYNOPSIS
    Lab cleanup: removes service, files, and resets student account state
...
# ── 2. Remove XAMPP directory ────────────────────────────────────────────────
Write-Host "[*] Removing $XAMPP_ROOT..."
if (Test-Path $XAMPP_ROOT) {
    Remove-Item -Path $XAMPP_ROOT -Recurse -Force
...
if (-not $dirCheck) { Write-Host "Directory $XAMPP_ROOT removed  : YES" -ForegroundColor Green }
else                { Write-Host "Directory $XAMPP_ROOT removed  : NO - check manually" -ForegroundColor Red }
